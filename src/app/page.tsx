...
import Cart from "./data/cartList/cart";
      ...
      <main>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mx-auto max-w-7xl px-2 mb-16" id="menu">
          {/* RENDERIZAÇÃO DO ARRAY DOS HAMBÚRGUERES */}
          {
            BurgersData
              .map((burger) => (
                <Foods key={burger.id} name={burger.name} image={burger.image} description={burger.description} price={burger.price} quantity={2}/>
              ))
          }
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mx-auto max-w-7xl px-2 mb-16" id="menu">
          {/* RENDERIZAÇÃO DO ARRAY DAS PIZZAS */}
          {
            PizzasData
              .map((pizza) => (
                <Foods key={pizza.id} name={pizza.name} image={pizza.image} description={pizza.description} price={pizza.price} />
              ))
          }
        </div>
        ...
        <div className=" grid grid-cols-1 sm:grid-cols-2 sm:grid-y-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-3 gap-y-3 mx-auto max-w-7xl px-2 mb-16" id="menu">
          {/* RENDERIZAÇÃO DO ARRAY DAS CERVEJAS */}
          {
            BeersData
              .map((beer) => (
                <Drinks key={beer.id} name={beer.name} image={beer.image} price={beer.price} type={beer.type} />
              ))
          }
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 sm:grid-y-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-3 gap-y-3 mx-auto max-w-7xl px-2 mb-16" id="menu">
          {/* RENDERIZAÇÃO DO ARRAY DOS REFRIGERANTES */}
          {
            SodasData
              .map((soda) => (
                <Drinks key={soda.id} name={soda.name} image={soda.image} price={soda.price} type={soda.type} />
              ))
          }
        </div>
